...
import edu.macalester.comp124.boundedQueue.*;
...
public class MazeGraph {
    ...
    */
     ...
     */
        ...
        DFSHelper(processedGraph.startNode, processedGraph.goalNode, processedGraph, path, visited);

        long end = System.nanoTime();
        System.out.println(end - start);
        return path;// remove this and put in your code.  Feel free to define more than one function (helpers are...helpful!)
    }

       public static ArrayList<Integer> DFSHelper(int currentNode,  int goalNode, ProcessedGraph processedGraph, ArrayList<Integer> Path, ArrayList<Integer> visited) {
           ...
           if (currentNode == goalNode) {
              ...
              for (Integer neighbor :  (List<Integer>) processedGraph.graph.getNeighbors(currentNode)) {
                  if (!visited.contains(neighbor)) {
                      ArrayList<Integer> currentNodeNeighbor = DFSHelper(neighbor, goalNode, processedGraph, Path, visited);
                      if (currentNodeNeighbor != null) {
                          return currentNodeNeighbor;
                      }
              }
              }
     ...
     */
        ...
        ArrayList<Integer> path = new ArrayList<Integer>(processedGraph.graph.getSize());
        queue.add(processedGraph.startNode);
        while (!queue.isEmpty()){
            Integer node = queue.pop();
            for(Integer neighbor : (List<Integer>) processedGraph.graph.getNeighbors(node)){
                if (neighbor == processedGraph.goalNode){
                    parents.put(neighbor, node);
                    Integer key = processedGraph.goalNode;
                    path.add(processedGraph.goalNode);
                    while(!path.contains(processedGraph.startNode)){
                        path.add(parents.get(key));
                        key = parents.get(key);
                    }
                    return path;
                }
    ...
    public static void main(String args[]) {
        testMaze("/Users/Reena/IdeaProjects/comp124s14/personal/src/edu/macalester/comp124/hello/maze1.txt");
       testMaze("/Users/Reena/IdeaProjects/comp124s14/personal/src/edu/macalester/comp124/hello/maze2.txt");
//        testMaze("maze3.txt");
//        testMaze("maze4.txt");
    }
